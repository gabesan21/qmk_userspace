...
- **2026-08-24:** **todo build é local — a CI (GitHub Actions) nunca é usada para compilar.** Motivo: os macros do teclado vão carregar variáveis sensíveis. Pendência de segurança a decidir na época dos macros: repo público não pode receber esses segredos — tornar o fork privado ou isolá-los num arquivo gitignored (ex.: `secrets.h`). Desativar/remover o workflow `build_binaries.yaml` fica proposto, ainda não executado.
